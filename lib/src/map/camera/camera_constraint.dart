...
abstract class CameraConstraint {
  ...
  /// Constrains the latitude of the camera to within [minLatitude] and [maxLatitude]
  ///
  /// This constraint does not affect the longitude of the camera.

  const factory CameraConstraint.latitudeBoundary({
    required double minLatitude,
    required double maxLatitude,
  }) = LatitudeBoundaryConstraint._;
}
...
@immutable
class LatitudeBoundaryConstraint extends CameraConstraint {
  final double minLatitude;
  final double maxLatitude;

  const LatitudeBoundaryConstraint._({
  ...
  MapCamera? constrain(MapCamera camera) {
    ...
    // Project the north and south bounds to pixel coordinates
    final northPixel =
        camera.project(LatLng(maxLatitude, testCenter.longitude), testZoom);
    final southPixel =
        camera.project(LatLng(minLatitude, testCenter.longitude), testZoom);

    final halfSize = camera.size / 2;

    // Calculate the permissible range for the map center's y-coordinate
    final topOkCenter = math.min(southPixel.y, northPixel.y) + halfSize.y;
    final botOkCenter = math.max(southPixel.y, northPixel.y) - halfSize.y;

    // Stop if the camera cannot be translated to stay within the latitude bounds
    if (topOkCenter > botOkCenter) return null;

    final centerPix = camera.project(testCenter, testZoom);
    final newCenterPix = Point(
      centerPix.x, // Keep the x-coordinate unchanged
      centerPix.y.clamp(topOkCenter, botOkCenter),
    );
  ...
  @override
  bool operator ==(Object other) {
    return other is LatitudeBoundaryConstraint &&
        other.minLatitude == minLatitude &&
        other.maxLatitude == maxLatitude;
  }

  @override
  int get hashCode => minLatitude.hashCode ^ maxLatitude.hashCode;
}
